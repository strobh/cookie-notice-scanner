...
class WebpageResult:
    ...
    def add_response(self, requested_url, mime_type):
...
class Crawler:
    ...
    def crawl_page(self, url):
        ...
        #self.find_cookie_notice_by_rules()
        #self.find_cookie_notice_by_cmp()
    ...
    def _event_response_received(self, response, **kwargs):
        ...
        self.result.add_response(requested_url=url, mime_type=mime_type)
    ...
    def find_cookie_notice_by_rules(self):
        ...
        for rule in self._rules:
            if self._is_rule_applicable(rule, self.result.hostname):
                node_ids = tab.DOM.querySelectorAll(nodeId=root_node_id, selector=rule.selector.get('value'))
                node_ids = node_ids['nodeIds']
                if len(node_ids) > 0:
    ...
    def find_cookie_notice_by_cmp(self):
